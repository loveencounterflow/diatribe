(function() {
  ...
  Programmatic_dialog = class Programmatic_dialog {
    ...
    _compile_steps(exp_steps) {
      /* TAINT validate */
      var R, answer, exp_modal, exp_ref, i, len;
      R = [];
      for (i = 0, len = exp_steps.length; i < len; i++) {
        ({exp_ref, exp_modal, answer} = exp_steps[i]);
        if (exp_ref == null) {
          exp_ref = null;
        }
        if (exp_modal == null) {
    ...
    async _step(modal, dlg_cfg) {
      var act_ref, answer, exp_modal, exp_ref, message, ref1;
